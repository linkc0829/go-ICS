...
if (JSON && JSON.stringify && JSON.parse) var Session = Session || (function() {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    (function () {
        ...
        async function createPortfolio(type) {
          ...
          data.type = (income.checked? INCOME: COST)
          ...
          await superagent
            ...
            .then(function (res) {            
              addPortfolio(type, res.body.portfolio);
            })
            .catch(function (err) {
              alert(err);
            });
  ...
  async function initPortfolio(user, type){
    ...
    document.querySelector('#cost-list').innerHTML = '';
    await superagent
        .get(target)
        .set('accept', 'json')
        .set('Authorization', jwt.token)
        .then(function (res) {
          console.log(res.body);
          let portfolio = res.body.Portfolios;
          for (let i = 0; i < portfolio.length; i++) {
            addPortfolio(portfolio[i], portfolio[i].type);
          }
        })
        .catch(function (err) {
            alert(err);
  ...
  function addPortfolio(res, type) {
    ...
    privacy_label.addEventListener('click', ()=>{ alert("Premium Feature: Privacy Control, Signup and Free Trial!");})
    ...
    voteBtn.addEventListener('click', (e)=>{ 
      e.preventDefault();
      alert("Premium Feature: Vote For " + type + ", Signup and Free Trial!"); 
    })
